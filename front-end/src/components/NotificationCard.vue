...
<template>
  <div
    class="relative flex items-start gap-2 p-4 rounded backdrop-blur border-white/10 bg-gradient-to-tr from-white/70 via-white to-white/90"
  >
    ...
    <div class="text-sm">
      ...
      <div class="leading-tight text-gray-500 capitalize-first">
        {{ notification.text }}
      </div>
      <div
        v-if="slots.footer"
        class="text-[10px] italic leading-tight text-gray-500 mt-2"
      >
        <slot name="footer" v-bind="{ payload: notification.payload }" />
      </div>
    </div>
    <button
      @click="close"
      class="absolute inline-flex transition top-3 right-3 text-zinc-300 hover:text-zinc-400 active:text-zinc-500 focus:text-blue-500 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
    >
      <IconX class="w-4 h-4" />
    </button>
  </div>
