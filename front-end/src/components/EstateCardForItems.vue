...
const { id, as, active, locked } = toRefs(props);
...
<template>
  <component
    :is="as"
    class="relative text-left transition bg-black/10 bg-gradient-to-tr from-white/10 rounded-2xl aspect-square m-2"
    :class="[active ? 'ring-2 ring-white shadow-xl' : 'showdow-sm']"
  >
    <div
    ...
    <div
      class="absolute bottom-0 left-0 grid w-full gap-2 rounded-b-2xl pb-2 px-2"
      v-if="isTokenReady"
    >
      <div>
